-- ═══════════════════════════════════════════════════════════════════════
-- DRISHTI — the one migration missing from the live database
-- ═══════════════════════════════════════════════════════════════════════
--
-- Probed against the live database on 2026-09-12 by checking, for each
-- migration, whether the columns and tables it creates exist. Result:
--
--   005, 008, 009, 010, 013, 014, 015, 016, 017, 018, 019 — APPLIED
--   007                                                   — NOT APPLIED
--
-- This file supersedes APPLY_MISSING_MIGRATIONS.sql, which claimed five were
-- missing. Four of those five were already in place; that file's probe had
-- tested the wrong table for two of them and inferred the rest from a single
-- failing write. Running it would have been harmless — every statement in it
-- is idempotent — but it asked you to read 571 lines to apply 67.
--
-- ## What 007 fixes
--
-- `calibration_snapshots.target_completion_rate` defaults to 0.1924, a figure
-- that does not match the published MPLADS completion rate. 007 corrects the
-- default to 0.5071, adds the by-count columns beside the by-value ones, and
-- makes the corpus-side columns nullable.
--
-- Until it runs, `computeCalibration` cannot write a snapshot: it sets
-- `corpus_completion_rate_by_count`, which does not exist yet. The insert fails
-- and the failure is swallowed into a console.warn, so /calibration has been
-- silently recomputing and discarding the same snapshot on every request.
--
-- ## Safety
--
-- Additive and idempotent: ADD COLUMN IF NOT EXISTS throughout, and DROP NOT
-- NULL on a column that is already nullable is a no-op. Re-running it changes
-- nothing.
--
-- The single DELETE removes rows written against the old 0.1924 benchmark.
-- `calibration_snapshots` currently holds **0 rows** (verified at probe time),
-- so it deletes nothing today. It is here so the file stays correct if applied
-- to a database where those rows do exist.
--
-- No other table is touched. No application data is at risk.
-- ═══════════════════════════════════════════════════════════════════════

-- 007_correct_completion_benchmark.sql
-- ===========================================================================
